...
export default function ItemPage() {
  ...
  // 행 편집용
  const [editKindPickerRowId, setEditKindPickerRowId] = useState(null);
  ...
  return (
    ...
    <Box>
            ...
            <Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <TextField
                  ...
                  onClick={() => setSearchKindPickerOpen(true)}
                  sx={{
                    cursor: "pointer",
                    "& .MuiInputBase-input": {
                      cursor: "pointer"
                    ...
                    {editMode && editingRowId === row.tempId ? (
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <TextField
                          ...
                          InputProps={{ readOnly: true }}
                          onClick={() => {
                            setEditKindPickerRowId(row.tempId);
                          }}
      ...
      {/* 🔍 검색용 */}
      <ItemKindPicker
        open={searchKindPickerOpen}
        onClose={() => setSearchKindPickerOpen(false)}
        onSelect={(selected) => {
          setSearch(prev => ({
            ...prev,
            itemKind: selected.code
          }));
          setSearchKindPickerOpen(false);
        }}
      />

      {/* ✏ 행 편집용 */}
      <ItemKindPicker
        open={Boolean(editKindPickerRowId)}
        onClose={() => setEditKindPickerRowId(null)}
        onSelect={(selected) => {
          handleCellChange(editKindPickerRowId, "item_kind", selected.code);
          setEditKindPickerRowId(null);
        }}
      />
